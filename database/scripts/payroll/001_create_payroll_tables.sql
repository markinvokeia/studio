...
-- NOTA: esta tabla se renombra a `payroll_retentions` en la migración 010 (Retenciones del empleado). La estructura no cambia.
...
-- ---------------------------------------------------------------------------
-- 16. payroll_egresos — Liquidaciones de egreso (desvinculaciones)
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS payroll_egresos (
  id                      UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  employee_id             UUID NOT NULL REFERENCES payroll_employees(id) ON DELETE RESTRICT,
  employment_id           UUID NOT NULL REFERENCES payroll_employments(id) ON DELETE RESTRICT,
  fecha_egreso            DATE NOT NULL,
  motivo                  VARCHAR(100) NOT NULL,
  salario_pendiente       DECIMAL(12,2) NOT NULL DEFAULT 0,
  aguinaldo_proporcional  DECIMAL(12,2) NOT NULL DEFAULT 0,
  licencia_no_gozada      DECIMAL(12,2) NOT NULL DEFAULT 0,
  salario_vacacional      DECIMAL(12,2) NOT NULL DEFAULT 0,
  indemnizacion           DECIMAL(12,2) NOT NULL DEFAULT 0,
  total                   DECIMAL(12,2) NOT NULL DEFAULT 0,
  created_at              TIMESTAMPTZ DEFAULT now(),
  created_by              UUID REFERENCES users(id) ON DELETE SET NULL
);

CREATE INDEX IF NOT EXISTS idx_payroll_egresos_employee   ON payroll_egresos(employee_id);
CREATE INDEX IF NOT EXISTS idx_payroll_egresos_employment ON payroll_egresos(employment_id);
COMMENT ON TABLE payroll_egresos IS 'Liquidaciones de desvinculación: salario pendiente, aguinaldo, licencia, salario vacacional e indemnización';


-- ---------------------------------------------------------------------------
-- TABLA 17: payroll_portal_requests — Solicitudes y reclamos del Portal del Empleado
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS payroll_portal_requests (
  id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  employee_id      UUID REFERENCES payroll_employees(id) ON DELETE SET NULL,
  user_id          UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  tipo             VARCHAR(20) NOT NULL CHECK (tipo IN ('solicitud', 'reclamo')),
  motivo           VARCHAR(50) NOT NULL CHECK (motivo IN ('recibos_sueldo', 'historial', 'novedades', 'certificados', 'datos', 'otro')),
  descripcion      TEXT NOT NULL,
  estado           VARCHAR(20) NOT NULL DEFAULT 'pendiente' CHECK (estado IN ('pendiente', 'respondida', 'cerrada')),
  respuesta        TEXT,
...
CREATE INDEX IF NOT EXISTS idx_portal_requests_user      ON payroll_portal_requests(user_id);
CREATE INDEX IF NOT EXISTS idx_portal_requests_employee  ON payroll_portal_requests(employee_id);
CREATE INDEX IF NOT EXISTS idx_portal_requests_estado    ON payroll_portal_requests(estado);
COMMENT ON TABLE payroll_portal_requests IS 'Solicitudes y reclamos enviados por empleados desde el Portal del Empleado';

DROP TRIGGER IF EXISTS trg_portal_requests_updated_at ON payroll_portal_requests;
CREATE TRIGGER trg_portal_requests_updated_at
  BEFORE UPDATE ON payroll_portal_requests
  FOR EACH ROW EXECUTE FUNCTION payroll_set_updated_at();


ALTER TABLE doctor_contracts
