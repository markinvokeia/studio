...
## Patrones UI — Reglas obligatorias (agregadas 2026-05-04)


### Listas con edición (CategoriesTable, ConceptsTable, WorkCalendarPanel, y todos los futuros)

**Patrón dos paneles:**
- Desktop: flex-row — lista a la izquierda (40%), formulario de edición a la derecha (60%) cuando hay ítem seleccionado
- Mobile: flex-col — lista de cards apiladas; al seleccionar, el formulario aparece debajo
- La lista usa cards clicables (`ring-2 ring-primary` para el seleccionado)
- El panel derecho tiene botones Guardar y Eliminar (destructivo) en el footer separado por `border-t`
- Botón × en el header del panel derecho para cerrar/deseleccionar

**Código de referencia:** [CategoriesTable.tsx](src/components/payroll/CategoriesTable.tsx)

```tsx
<div className="flex flex-col md:flex-row gap-4">
  {/* Lista */}
  <div className={cn('flex flex-col gap-2', selected ? 'md:w-2/5' : 'w-full')}>
    {items.map((item) => (
      <Card onClick={() => select(item)} className={cn(selected?.id === item.id && 'ring-2 ring-primary')}>
        ...
      </Card>
    ))}
  </div>
...
- Siempre `<DialogContent className="max-w-md">` o similar
- El contenido del form lleva `<div className="flex flex-col gap-4 p-1">` (padding interno `p-1` mínimo, para que el contenido no quede pegado a los bordes del dialog)
- El `DialogFooter` queda fuera del div de contenido

### Cards lado a lado (Tasas y Parámetros)

- Grid `grid grid-cols-1 sm:grid-cols-2 gap-4` → 50%/50% en desktop, 100% en mobile
- Cada card tiene su propio botón Save en el `CardFooter` con estado `saving` independiente por sección
- Código de referencia: [PayrollSettings.tsx](src/components/payroll/PayrollSettings.tsx)


### Estado de carga

- Siempre mostrar `<Skeleton>` mientras carga, nunca tabla vacía ni spinner genérico
- Patrón: `{loading ? <Skeleton... /> : <contenido real>}`


### Endpoints de settings (payroll/settings tab)

| Tab | Endpoints |
|-----|-----------|
| Tasas y Parámetros | GET/POST `/payroll/settings` + `/payroll/settings/update` |
| Conceptos | GET/POST `/payroll/concepts` + `/payroll/concepts/upsert` + `/payroll/concepts/delete` |
| Categorías Grupo 15 | GET/POST `/payroll/categories` + `/payroll/categories/upsert` + `/payroll/categories/delete` |
| Calendario Laboral | GET/POST `/payroll/calendar` + `/payroll/calendar/upsert-event` + `/payroll/calendar/delete-event` |


### DB field names (no usar los nombres del mock-data)

| Componente | Campos DB reales |
|------------|-----------------|
| CategoriesTable | `id`, `name`, `description`, `min_salary_bpc`, `max_salary_bpc`, `is_active` |
| ConceptsTable | `id`, `code`, `name`, `concept_type`, `affects_bps`, `affects_irpf`, `is_active` |
| WorkCalendarPanel | `id`, `event_date`, `event_type` (enum: `feriado_nacional`, `feriado_departamental`, `cierre_clinica`, `dia_puente`), `description` |
