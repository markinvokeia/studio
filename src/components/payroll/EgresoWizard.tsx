...
export function EgresoWizard({ initialEmployee, initialEmployments, onClose, onDone }: EgresoWizardProps = {}) {
  ...
  async function handleCalculate() {
    ...
    // Licencia no gozada: use the real accrued vacation balance when available,
    // falling back to a prorated 20-day estimate for the standalone/mock case.
    let diasLicencia = Math.round(Math.min(yearsWorked, 1) * 20 * 100) / 100;
    const empId = selectedEmployee || initialEmployee?.id;
    if (empId) {
      try {
        const res = await api.get(API_ROUTES.PAYROLL.VACATION_BALANCE, { employee_id: empId });
        const bal = ((res?.data ?? res) ?? null) as { saldo?: number } | null;
        if (bal && typeof bal.saldo === 'number' && bal.saldo > 0) {
          diasLicencia = Math.round(bal.saldo * 100) / 100;
        }
    ...
    <div className="flex flex-col gap-6 p-4 sm:p-6 max-w-2xl">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex gap-2">
            ...
            <Button disabled={confirming} onClick={async () => {
              if (!result || !selectedEmployment) return;
              try {
                setConfirming(true);
                await api.post(API_ROUTES.PAYROLL.EMPLOYEES_EGRESO, {
                  employee_id: selectedEmployee || initialEmployee?.id,
                  employment_id: selectedEmployment,
                  fecha_egreso: fechaEgreso,
                  motivo,
                  ...result,
                });
              } catch { /* proceed anyway — UI still shows confirmation */ }
              setConfirming(false);
              setStep(3);
            }}>{confirming ? '...' : t('confirmAndGenerate')}</Button>
          </div>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="flex flex-col items-center gap-4 py-10 text-center">
          ...
          <div className="flex gap-2">
            {initialEmployee ? (
              <Button variant="outline" onClick={() => { onDone?.(); onClose?.(); }}>{t('confirmed.close')}</Button>
            ) : (
