...
// Postgres/n8n returns numeric columns as strings ("134250.00", "0.0900").
// Coerce plain numeric strings to numbers so the component math/formatting works.
// UUIDs, ISO dates, currency codes and other non-numeric strings are left intact.
const NUMERIC_STRING = /^-?\d+(\.\d+)?$/;

export function coerceNumericStrings<T extends Record<string, unknown>>(row: T): T {
  const out: Record<string, unknown> = { ...row };
  for (const key in out) {
    const value = out[key];
    if (typeof value === 'string' && value.trim() !== '' && NUMERIC_STRING.test(value.trim())) {
...
// Returns a YYYY-MM-DD string for <input type="date"> from any backend date
// value (ISO datetime "1990-05-01T00:00:00.000Z" or plain "1990-05-01").
export function toDateInput(dateStr?: string | null): string {
  if (!dateStr) return '';
  return dateStr.slice(0, 10);
}

// Extracts HH:mm from an ISO datetime without timezone conversion
// (matches the formatDate slice convention to avoid hour shifting).
