    ...
    const handleSaveAppointment = async (appointmentDetails: Partial<Appointment> & { colorId?: string }) => {
        ...
        if (isEditing) {
            ...
            if (hasChanges && availabilityStatus !== 'available') {
                toast({
                    variant: "destructive",
                    title: tToasts('slotUnavailableTitle'),
                    description: tToasts('slotUnavailableDescription'),
                });
                return;
            }
        }
        ...
        try {
            ...
            if (result.code === 200 || result.status === 'success') {
                ...
                toast({
                    title: successTitle,
                    description: result.message || tToasts('appointmentSavedSuccess', { action: isEditing ? 'updated' : 'saved' }),
                });
                ...
                if (errorMessage.includes("No existe disponibilidad")) {
                    toast({
                        variant: "destructive",
                        title: tToasts('slotUnavailableTitle'),
                        description: tToasts('slotNoLongerAvailable'),
                    });
                    checkAvailability(newAppointment);
                } else {
                    toast({
