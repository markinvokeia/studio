...
function InvoicesPageContent() {
    ...
    const loadCreditNotes = React.useCallback(async () => {
        ...
        setCreditNotes(await getCreditNotesForInvoice(selectedInvoice.id, selectedInvoice.user_id, false));
    ...
    const loadAllocations = React.useCallback(async () => {
        ...
        const data = await api.get(API_ROUTES.PURCHASES.INVOICE_ALLOCATIONS, { credit_note_id: selectedInvoice.id });
    ...
    const handlePrintInvoice = async (invoice: Invoice) => {
        ...
        try {
            ...
            toast({
                ...
                description: `Your PDF for Invoice #${fileName} is downloading.`,
            });
                    ...
                    leftPanel={
                        <InvoicesTable
                            ...
                            filterOptions={[
                                ...
                                { label: t('creditNote'), value: "credit_note" },
                            ]}
                            isSales={false}
                        ...
                        selectedInvoice && (
                            <Card className="h-full border-0 lg:border shadow-none lg:shadow-sm flex flex-col min-h-0">
                                ...
                                <div className="px-6 py-3 flex items-center gap-2 flex-wrap border-b bg-muted/30">
                                    ...
                                    {canAddPaymentAction && (
                                        <>
                                            <Button variant="default" size="sm" className="h-8 gap-1.5 text-xs" onClick={() => openBillingWizard({ invoiceId: selectedInvoice.id, invoice: selectedInvoice, patientId: selectedInvoice.user_id, patientName: selectedInvoice.user_name, isSales: false }, () => { loadInvoices(); loadPayments(); })}>
                                            ...
                                            <div className="m-0 h-full flex flex-col">
                                                <div className="flex-1 min-h-0">
                                                    <InvoiceItemsTable
                                                        ...
                                                        onCreate={canEditItems ? handleCreateItem : undefined}
