...
const upsertTemplate = async (data: any) => {
    try {
        return await api.post(API_ROUTES.MEDICAL_INSTRUCTION_TEMPLATES_UPSERT, data);
    } catch (error) {
...
const deleteTemplate = async (id: string) => {
    try {
        return await api.delete(API_ROUTES.MEDICAL_INSTRUCTION_TEMPLATES_DELETE, { id });
    } catch (error) {
...
async function getTemplates(params: { search?: string; page?: number; limit?: number } = {}): Promise<{ data: MedicalInstructionTemplate[]; total: number; page: number; limit: number }> {
    try {
        const query: Record<string, string> = {};
        if (params.search) query.search = params.search;
        if (params.page) query.page = params.page.toString();
        if (params.limit) query.limit = params.limit.toString();
        const response = await api.get(API_ROUTES.MEDICAL_INSTRUCTION_TEMPLATES, query);
