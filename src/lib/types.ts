...
export interface DocPrintTemplate {
  id: string;
  clinic_id: string;
  template_type: 'quote' | 'invoice' | 'payment' | 'credit_note' | 'prepayment' | 'financial_summary' | 'caja_apertura' | 'caja_cierre' | 'caja_sesion' | 'payroll_period' | 'payroll_receipt' | 'payroll_report';
...
// Mirrors the DB `contract_type_enum` (payroll_contracts.contract_type).
...
export interface PayrollEntry {
  ...
  /** Exact rates/caps and per-employee inputs used by the calc engine (transparency). */
  calculation_params?: PayrollCalculationParams;
}

/** Snapshot of the engine settings + per-employee inputs captured when an entry was calculated. */
export interface PayrollCalculationParams {
  ...
  fonasa_rate_applied?: number;
  fonasa_family_situation?: FonasaFamilySituation;
  /** Sum of the employee's active IRPF deductions (monto_mensual) at calc time. */
  irpf_deductions_monthly?: number;
  /** DGI tiered deduction rate applied (0.10 / 0.08). */
  irpf_deduction_rate?: number;
...
export type PayrollDocumentEstado = 'pendiente' | 'generado' | 'error' | 'no_procede';

/** Tracking row for a generated payroll report/document (see payroll_documents). */
export interface PayrollDocument {
  id: string;
  payroll_period_id: string;
  clinic_id?: number;
  tipo: PayrollDocumentTipo;
  estado: PayrollDocumentEstado;
  formato?: string;
  generated_at?: string;
  generated_by?: string;
  error_message?: string;
...
export interface PayrollAusencia {
  id: string;
  employee_id: string;
  employee_name?: string;
  user_id?: string;
...
export interface PayrollClinicalSession {
  id: string;
  start_at: string;            // ISO datetime (fecha_sesion)
  end_at?: string | null;      // ISO datetime from linked appointment, if any
...
// ── Portal del Empleado — Solicitudes ─────────────────────────────────────────
export type PortalRequestTipo = 'solicitud' | 'reclamo';
export type PortalRequestMotivo =
  | 'recibos_sueldo'
  | 'historial'
  | 'novedades'
  | 'certificados'
  | 'datos'
  | 'otro';
export type PortalRequestEstado = 'pendiente' | 'respondida' | 'cerrada';

export interface PortalRequest {
  id: string;
  employee_id?: string;
  user_id: string;
  tipo: PortalRequestTipo;
  motivo: PortalRequestMotivo;
  descripcion: string;
  estado: PortalRequestEstado;
  respuesta?: string;
  respondido_por?: string;
  respondido_por_name?: string;
